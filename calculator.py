...
class Calculator:
    ...
    def bind_keys(self):
        ...
        for key in self.digits:
            self.window.bind(str(key), lambda event, digit=key: self.add_to_expresion(digit))
    ...
    def create_digit_buttons(self):
        for digit, grid_value in self.digits.items():
            button = tk.Button(self.buttons_frame, text=str(digit), bg = WHITE, fg=LABEL_COLOR,
                               font=DIGITS_FONT_STYLE, borderwidth = 0, command =lambda x=digit: self.add_to_expresion(x))
            button.grid (row = grid_value[0], column = grid_value[1], sticky=tk.NSEW)


    def append_operator(self, operator):
        self.current_expression = str(self.current_expression)
        if self.current_expression[-1] != operator:
    ...
    def update_label(self):
        self.label.config(text=str(self.current_expression)[:11])
